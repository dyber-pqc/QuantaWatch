...
use qw_scanner::{CryptoAssetType, PqcStatus};
...
pub fn build_graph(
    ...
    let providers: BTreeMap<String, (PqcStatus, String, String)> = state
        .provider_crypto
        .iter()
        .map(|e| {
            let v = e.value();
    ...
    // Identities (CIEM): config users + API keys.
    let mut identities: Vec<(String, String)> = Vec::new(); // (label, role)
    for u in &state.config.auth.users {
        if u.org == tenant || tenant == qw_store::DEFAULT_TENANT {
            identities.push((u.username.clone(), u.role.clone()));
        }
    }
    for k in &state.config.auth.api_keys {
        if k.org == tenant || tenant == qw_store::DEFAULT_TENANT {
            identities.push((format!("apikey:{}", k.name), k.role.clone()));
        }
    }
    ...
    for a in state.store.list_assets(tenant) {
