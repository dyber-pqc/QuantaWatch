...
//! the leaf's DER, produced by the gateway's FIPS-204 identity. A relying party
//! that understands the binding gets post-quantum authentication; a classical
//! client still gets a working X.509 chain. This is what lets QuantaWatch
//! *issue* the replacement for a quantum-vulnerable certificate — completing
//! remediation past "open a PR" — rather than only recommending one.
//!
...
/// The internal CA: a persisted classical Ed25519 root plus the gateway's
/// ML-DSA-65 identity for the post-quantum binding.
...
impl CertAuthority {
    ...
    /// persist a fresh one. The ML-DSA binding uses the gateway identity.
