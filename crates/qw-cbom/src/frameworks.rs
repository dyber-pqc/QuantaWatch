//! Multi-framework compliance controls (CNSA 2.0, NIST SP 800-53, PCI-DSS,
//! FedRAMP), evaluated against a QuantaWatch deployment. Shared by the gateway
//! (live config) and the desktop app. Callers pass a [`Signals`] snapshot of
//! what the deployment actually enforces; the control status reflects reality.
...
/// Live capability signals — what the deployment actually enforces. Build this
/// from live config (gateway) or from local signals (desktop).
pub struct Signals {
    pub auth_on: bool,
    pub lockout: bool,
...
pub fn all(s: &Signals) -> Vec<Framework> {
    vec![cnsa(s), nist_800_53(s), pci_dss(s), fedramp(s)]
}

pub fn summarize(f: &Framework) -> serde_json::Value {
